...
namespace Editor
{
    class Inspector_Blackboard : Inspector
    {
        ...
        public override void Inspect(Control Container, object Parameter1, object Parameter2, object Parameter3)
        {
            ...
            foreach (BlackboardData Blackboard in BlackboardManager.GetInstance().Blackboards.Values)
            {
                ComboBox.Items.Add(Blackboard.Name);
                if(Blackboard.Name == Value)
                {
                    ComboBox.SelectedIndex = ComboBox.Items.IndexOf(Blackboard.Name);
                }
            }
